...
export default function FreightForwardEtaUpdaterPage() {
  ...
  const searchVessel = useCallback(async (name: string) => {
    ...
    setSearching(true);
    try {
      const rows = await findFreightByVesselName(trimmed);
      setMatches(rows);
    } catch {
      setError("Unable to search jobs for this vessel.");
      setMatches([]);
    } finally {
      setSearching(false);
    }
  ...
  const handleUpdate = async () => {
    ...
    if (!matches.length) {
      setError("No jobs found for this vessel name.");
      return;
    }

    setUpdating(true);
    try {
      const result = await updateEtaByVesselName(
        name,
        eta,
        user?.username ?? "unknown"
      );
      setMessage(`Updated ETA to ${eta} on ${result.updated} job(s).`);
      const rows = await findFreightByVesselName(name);
      setMatches(rows);
    } catch {
  ...
  return (
    <div className="rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm">
      <ModuleHeader
        title="ETA Updater"
        description="Find freight jobs by vessel name and update ETA for all matching jobs at once."
      />
      ...
      <div className="mt-5 overflow-x-auto rounded-xl border border-zinc-200">
        <table className="min-w-full text-left text-xs">
          <thead className="bg-zinc-50 text-[10px] uppercase tracking-wide text-zinc-500">
            <tr>
              <th className="px-3 py-2 font-semibold">FF No</th>
            ...
            {!vesselName.trim() ? (
              <tr>
                <td colSpan={6} className="px-3 py-8 text-center text-zinc-400">
                  Enter a vessel name to list matching jobs.
                </td>
              </tr>
            ) : searching ? (
              <tr>
              ...
              <tr>
                <td colSpan={6} className="px-3 py-8 text-center text-zinc-400">
                  No active jobs found for this vessel name.
                </td>
              </tr>
            ) : (
              matches.map((item) => (
                <tr key={item.id} className="border-t border-zinc-100">
                  ...
                  <td className="px-3 py-2 text-zinc-700">{item.consignmentName}</td>
                  <td className="px-3 py-2 text-zinc-700">{item.vesselName || "—"}</td>
